...
class SensorScanViewController: UIViewController, UIGestureRecognizerDelegate, CBPeripheralManagerDelegate{
...
extension SensorScanViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        let recipes = RecipeSensorScan.createRecipes()
        return recipes.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "SensorTableCell") as? SensorTableCell {
            let recipesSensorScan = RecipeSensorScan.createRecipes()
            cell.configurateTheCellSensorScan(recipesSensorScan[indexPath.row])
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        if MyVariables.sensorChoose == 0{
            MyVariables.sensor1 = MyVariables.sensors[indexPath.row]
        }else if MyVariables.sensorChoose == 1{
            MyVariables.sensor2 = MyVariables.sensors[indexPath.row]
        }else if MyVariables.sensorChoose == 2{
            MyVariables.sensor3 = MyVariables.sensors[indexPath.row]
        }else if MyVariables.sensorChoose == 3{
            MyVariables.sensor4 = MyVariables.sensors[indexPath.row]
        }
